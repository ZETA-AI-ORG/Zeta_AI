#!/usr/bin/env python3
"""\
🧪 JESSICA RAG SIMULATOR

Simule une conversation sur l'endpoint /jessicaragbot (Jessica - RAG sophistiqué):
- Modèle: résolu par Système C (Gemma 4-26B / 4-31B / Gemini 3.1) selon plan
- Prompt: prompt_universel_v2.md avec catalogue dynamique
- Rôle: Vendeuse experte catalogue + calcul prix + validation commande

Legacy purpose (rag_simulator original):
...
Usage:
    python tests/jessica_simulator.py                 # interactif
    python tests/jessica_simulator.py --scenario      # scénario court (11)
    python tests/jessica_simulator.py --scenario58    # pack questions (58)
    python tests/jessica_simulator.py --scenario120   # pack production_test_cases (si dispo)

Config (env):
    JESSICA_CHAT_URL  (default: http://127.0.0.1:8001/jessicaragbot)
...
async def _resolve_chat_url() -> str:
    ...
    env_url = os.getenv("JESSICA_CHAT_URL") or os.getenv("RAG_CHAT_URL") or os.getenv("CHAT_URL")
    candidates = [u for u in [env_url, "http://127.0.0.1:8001/jessicaragbot", "http://127.0.0.1:8002/jessicaragbot"] if u]
    ...
    async with httpx.AsyncClient(timeout=2.5) as client:
        for url in candidates:
            try:
                # Support /jessicaragbot et /chat (legacy)
                base = url.rsplit("/jessicaragbot", 1)[0].rsplit("/chat", 1)[0]
                # Try OpenAPI first
    ...
    return candidates[0] if candidates else "http://127.0.0.1:8001/jessicaragbot"


CHAT_URL = "http://127.0.0.1:8001/jessicaragbot"  # resolved at runtime in main()
...
TEST_USER_ID = f"test_jessica_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}_{uuid.uuid4().hex[:6]}"
...
class JessicaSimulator:
    ...
    def _json_report_path(self) -> Path:
        ...
        return out_dir / f"jessica_simulator_{ts}_{TEST_USER_ID}.json"
    ...
    async def send_message(self, message: str, images: list | None = None) -> dict | None:
        ...
        # 🤖 JESSICA: pas de botlive_enabled (endpoint dédié /jessicaragbot)
        payload = {
            "company_id": TEST_COMPANY_ID,
            "user_id": TEST_USER_ID,
            "message": message,
        ...
        try:
            if not self.use_http:
                ...
                # 🤖 JESSICA: appeler l'endpoint dédié jessica_ragbot_endpoint
                from Zeta_AI.app import jessica_ragbot_endpoint as chat_endpoint
    ...
    async def run_scenario(self) -> None:
        ...
        # CSV
        try:
            out_path = Path("tests/jessica_scenario_results.csv")
...
async def main() -> None:
    ...
    print("\n" + "=" * 80)
    print(f"� JESSICA SIMULATOR (/jessicaragbot) - TEST_USER_ID: {TEST_USER_ID}")
