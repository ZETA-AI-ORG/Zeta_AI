...
async def analyze_product_with_gemini(
    *,
    ...
    prompt = (
        ...
        "(1) si c'est une photo de PRODUIT à commander, identifier le TYPE de produit (catégorie générique); "
        "(2) si c'est une CAPTURE DE PAIEMENT (Wave/Orange Money/MTN Money/etc.), appliquer la MÊME logique que notre OCR strict. "
        "\n\n"
        "IMPORTANT (produit - scalabilité / anti-marque):\n"
        "- Pour un PRODUIT: le champ name doit être une catégorie générique (1 à 4 mots, en français), ex: 'couches bébé', 'lait infantile', 'riz', 'huile', 'sucre', 'savon', etc.\n"
        "- INTERDIT dans name: marque, modèle, taille (S/M/L), quantité (ex: '60'), grammage/volume (ex: '5L', '900g'), ou toute info d'emballage.\n"
        "- Si une marque est visible: mets-la uniquement dans notes (ex: 'marque visible: XXX').\n"
        "- Si tu hésites sur la marque mais tu es sûr du type: privilégie le type générique et mets l'incertitude dans notes.\n"
        "- Objectif: permettre à un autre système (catalogue/LLM) de clarifier ensuite (variantes, taille, etc.) sans bloquer sur la marque.\n"
        "\n"
    ...
    prompt += (
        ...
        "\n"
        "Règles:\n"
        "- Si produit: is_product_image=true, name non vide (catégorie générique), confidence élevée, payment=null.\n"
