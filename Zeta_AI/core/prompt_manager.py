...
class PromptManager:
    ...
    async def get_active_prompt(self, company_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if result.data:
                ...
                logger.info(f"[PROMPT] Active prompt v{prompt_data['version']} for company {company_id}")
                return prompt_data
            else:
                ...
                logger.warning(f"[PROMPT] No prompt found for company {company_id}")
                return None
                
        except Exception as e:
            logger.error(f"[ERROR] Failed to get active prompt: {e}")
            return None
    ...
    async def create_prompt_version(
        ...
        try:
            ...
            logger.info(f"[PROMPT] Created v{next_version} for company {company_id} by {created_by}")
            return result.data[0]
            
        except Exception as e:
            logger.error(f"[ERROR] Failed to create prompt version: {e}")
            raise
    ...
    async def rollback_to_version(self, company_id: str, target_version: int, created_by: str) -> bool:
        ...
        try:
            ...
            if not version_check.data:
                logger.warning(f"[PROMPT] Version {target_version} not found for company {company_id}")
                return False
            ...
            logger.info(f"[PROMPT] Rolled back to v{target_version} for company {company_id} by {created_by}")
            return True
            
        except Exception as e:
            logger.error(f"[ERROR] Failed to rollback prompt: {e}")
            return False
    ...
    async def get_prompt_history(self, company_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"[ERROR] Failed to get prompt history: {e}")
            return []
    ...
    async def ab_test_prompt(self, company_id: str, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"[A/B] User {user_id} -> prompt v{selected_prompt['version']}")
            
            return selected_prompt
            
        except Exception as e:
            logger.error(f"[ERROR] A/B test prompt selection failed: {e}")
            return await self.get_active_prompt(company_id)
